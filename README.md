...
教师和学生登录页默认启用“记住账号”，但页面打开时保持空白。点击姓名或工号/学号字段后，可从当前角色最近成功登录的账号历史中选择，系统再同时填写姓名与编号。历史按编号去重，每个角色最多保留 5 项，使用版本化键 `oa.auth.account-history.v2`。密码、Cookie、Session ID 和 Token 不写入 Web Storage，密码保存与自动填充由浏览器密码管理器负责。
