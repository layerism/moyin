...
3. TOCTOU 对接先新增发布 payload 与 409 映射测试；首次运行因 `createPublishRequestPayload` 尚未导出而失败，完成 helper 后 focused tests 7/7 通过。
4. 最终全量前端测试 21/21 通过。
...
- 对接 `240d5cc` TOCTOU 约束：`RevisionImpact.draftConfigHash` 为必填；`workflowApi.publish()` 始终发送 `{ expectedDraftConfigHash }`，首次发布为 `null`，重新发布为预检返回的 hash。
...
- 重新发布确认不再重复保存草稿，而是直接使用 impact hash 发布，保留后端对预检与发布之间草稿变化的检测能力；确认请求收到 409 时关闭旧 impact，显示“草稿已变化，请重新预览影响”，且不显示成功状态。
- 影响弹窗显示当前/下一版本、内容变更节点数、新增节点数、需重新提交节点数和受影响学生数，并明确说明受影响提交仅作为审计历史保留。
- 发布态节点检查器中的 deadline 字段只读，并引导至“填写进度”；运行时 deadline 更新不写回修订内容。

## 验证结果

- `cd frontend && node --experimental-strip-types --test tests/*.test.ts`：21/21 通过。
