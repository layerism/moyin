...
### Task 7: 自动刷新草稿文件节点的兼容脚本配置

进一步诊断发现，草稿中三个普通文件节点的脚本代码哈希仍与当前版本一致，但配置哈希已经过期。确认节点刷新后，统一校验继续在这些文件节点处拒绝保存。

- [x] 仅当文件节点的脚本代码哈希仍匹配、配置哈希发生变化时自动刷新。
- [x] 刷新配置哈希、允许扩展名和运行设置，保留节点自己的参数值。
- [x] 脚本代码哈希变化、脚本版本失效或节点参数不再符合新定义时，继续由原有校验拒绝保存。
- [x] 已发布版本快照保持不可变，只更新待保存的草稿配置。
