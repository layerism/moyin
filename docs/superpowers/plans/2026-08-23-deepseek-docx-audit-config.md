...
- [ ] 先调整测试输入和请求行为断言，使其描述 `DEEPSEEK_MODEL` 环境契约。
- [ ] 从 DOCX 审核脚本配置及设置校验中删除 `modelName`。
- [ ] 让请求函数校验并读取 `DEEPSEEK_MODEL`，不提供回退模型。
- [ ] 更新后端设置、示例环境文件和本地环境文件的白名单。
- [ ] 静态检查测试、脚本配置、环境变量传递和差异，不执行测试或浏览器检查。
- [ ] 创建结果检查点，仅提交本任务跟踪文件；本地重启后端并核对 8000 端口。
