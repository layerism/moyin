...
**Architecture:** 新协议使用 `schemaVersion="2.0"` 与 `graderVersion="answer-sheet-v2"`，并用 `format="single_markdown_exact"` 区分新填空题；v2 仍可承载原样保留的 v1 填空题，使修订流程可以混用新旧题。前端在可编辑草稿中仅安全升级单空、单答案、非大小写敏感的旧题；后端按协议与题目格式分发校验、提交归一化和确定性判分。
...
- [ ] **Step 1: 将填空题类型拆成旧版与单 Markdown 联合类型**
...
对每个旧填空题，只有在恰好一个 blank、`caseSensitive === false`、最多一个非空有效答案、标记恰好出现一次时，才转换为新结构；题干使用 `content.replace(marker, "")`，分值取原 blank 分值，标准答案取唯一非空答案或空字符串。未满足条件的旧题原样保留。只要进入可编辑草稿，返回契约统一写成 v2；输入对象不得原地修改。

- [ ] **Step 5: 在编辑入口使用升级后的契约**
