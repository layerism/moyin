# Student Deadline Modal Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the clipped table-row deadline editor with a centered secondary modal whose fields and action buttons remain fully visible.

**Architecture:** Keep the existing `TeacherProgressPanel` state, validation, API call, and feedback flow. Move only the editor rendering out of the horizontally scrollable table into a full-viewport sibling overlay, then add local focus restoration and body-scroll locking without introducing a shared modal abstraction.

**Tech Stack:** React 18, TypeScript, native HTML dialog semantics, CSS Grid/Flexbox, Vite.

## Global Constraints

- Work on the current branch.
- Preserve the existing user changes in `AGENTS.md`, `docs/05_oa_graph.md`, and `.superpowers/`.
- Modify only `frontend/src/features/academic-flow/TeacherProgressPanel.tsx` and `frontend/src/styles.css`.
- Do not modify backend files, API signatures, progress data types, database state, or deadline validation rules.
- Do not add dependencies, shared modal abstractions, or new source files.
- The secondary modal backdrop must block pointer input, scrolling, dragging, and zooming on the progress panel, page, and flow canvas.
- Backdrop clicks must not close the modal; only the explicit close button and “取消” may close it.
- The modal body is the only scrollable region; the title and action areas remain visible.
- Desktop fields use two columns; narrow screens use one column without clipped controls or button text.
- Do not run automated tests, builds, browser automation, or Browser-plugin validation during implementation.
- Perform only static code-flow inspection, cache cleanup outside `backend/.venv`, local service restart, and listener checks.
