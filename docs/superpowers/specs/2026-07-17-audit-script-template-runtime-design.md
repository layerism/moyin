...
无论执行成功、失败或超时，都必须在 Runner 将结果或错误复制到内存后清理该目录；调用审核阶段负责持久化返回结果。
...
8. 校验输出 JSON Schema；
9. 返回经过协议校验的 `AuditResult`，由调用审核阶段持久化；
10. 清理临时目录。
...
- 调用方结果持久化失败：Runner 临时目录已经清理，调用方记录持久化错误并按审核状态机重试；
...
7. Runner 能解析固定脚本版本、执行脚本、校验并返回结果、清理临时目录。
