...
> 状态：待用户审阅。本文只定义目标架构和业务语义，不表示功能已经实现。
...
- 超级管理员需要直接修改脚本源码和全局配置，保存后立即对线上新审核生效；
...
2. 超级管理员可以在现有“审核脚本”管理界面修改源码、基本信息、参数默认值和运行配置。
...
- 不允许普通教师编辑脚本源码、全局运行配置或脚本处理能力。
- 不允许发布后更换节点的脚本类型、模板、文件类型或流程拓扑。
- 不在本阶段支持多个 Uvicorn 进程或多台服务器共同消费 SQLite 队列。
- 不把脚本子进程描述为安全沙箱；源码编辑权限只授予可信超级管理员。
- 不增加外部队列、Redis、Celery 或文件系统 watcher。

## 3. 方案选择

### 3.1 采用：管理 API 激活、内部代际校验、单进程 WorkerPool

管理接口先校验完整的新源码与配置，再把脚本状态切换为 `updating`，暂停领取该脚本的新任务，完成文件替换和数据库状态更新后切回 `ready`。每次成功激活只增加内部 `generation`，不产生可选择的历史版本。
...
文件 watcher 无法可靠判断编辑器是否已经写完源码和配置，也无法把磁盘替换、任务取消和数据库代际更新组合成明确的业务事务。管理 API 是唯一正式写入口；部署时的外部文件变化只在服务启动扫描中处理。
...
第一阶段管理端只允许修改已有参数的默认值、已有运行配置的值、`maxConcurrency` 和源码。参数键、类型、范围、脚本语言、入口文件名及文件扩展名能力仍由仓库结构定义，避免一次热更新使全部节点配置失去结构兼容性。
...
- 超级管理员修改源码或全局配置：按 `script_id` 取消所有教师、所有流程和所有节点中尚未完成的相关任务；
...
管理详情另外返回即时计算的 `editor_hash`，它覆盖名称、说明、源码和配置，用于防止两个管理员互相覆盖。只修改名称或说明不会增加 generation，也不会取消审核任务；源码或 `config.json` 任一内容变化才构成一次热激活。
...
4. 运行配置；
5. 最大并发数；
6. 当前状态、内部 generation、最后更新时间。

界面不出现版本号。源码编辑区按脚本语言显示纯文本，保存时提交完整源码；不在浏览器中暴露服务器绝对路径、环境密钥或其他脚本文件。
...
`expectedEditorHash` 不一致返回 `409`，防止两个超级管理员互相覆盖。源码或配置不合法返回 `422`，磁盘写入失败返回 `500`。
...
2. 在临时文件中生成完整源码和配置；
3. 执行源码大小、UTF-8、Python/JavaScript 语法、配置结构及参数值校验；
4. 把脚本状态改为 `updating`，使领取查询暂时排除该脚本；
5. 在一个 SQLite 事务中取消该脚本全部 `pending`、`running` 任务并记录原因 `script_updated`；
6. 通知本进程正在运行的相关子进程停止；
7. 使用 `os.replace` 分别替换源码、配置和发生变化的 manifest；替换期间脚本保持 `updating`，worker 不会读取半更新状态；
8. 重新从磁盘解析并计算哈希；
9. generation 递增，写入新哈希并把状态改为 `ready`；
10. 返回服务器重新读取后的完整详情。

若第 7 至第 9 步失败，脚本保持 `error`，不接受新任务。管理员修正并再次成功保存后才能恢复 `ready`。系统不静默退回旧源码，也不把部分更新宣称为成功。

如果请求只改变名称或说明，则校验后直接原子替换 manifest 并返回，不进入 `updating`、不增加 generation，也不取消任务。

语法检查只能排除基础错误，不能保证业务逻辑正确。新脚本运行异常仍按现有“审核服务异常”处理，不能伪装成业务审核不通过。
...
2. 超级管理员修改源码或全局配置并保存后，不重启服务即可让新提交使用新内容。
...
13. 普通教师不能读取或修改脚本源码和全局配置，超级管理员不能通过脚本接口访问脚本目录以外的文件。
